...
<div class="pageHeader">
	<form id="pagerForm" onsubmit="return navTabSearch(this);"
		action="signPn_listHotThemePn.action" method="post">
		<!-- 分页表单参数 -->
		<%@include file="/page/inc/pageForm.jsp"%>
		<div class="searchBar">
			<table class="searchContent">
				<tr>
				<li><a href="hotThemePn_readyInsert.action" title="添加"
		...
		<tbody>
			<s:iterator value="recordList" status="st">
				<tr target="sid_user" rel="${id}">
					...
					<td>${hotThemePn}</td>
					<td><img src="${hotThemePnPic}" style="width: 100px;height: 100px;"/></td>
						<td><c:if test="${status==100}">已上架</c:if> <c:if
							test="${status==101}">已下架</c:if></td>
					<td>
					<c:if test="${status==100}">[<a
								href="hotThemePn_changeStatus.action?id=${id}&status=101"
								title="确认【下线】" target="ajaxTodo" style="color: blue">强制冻结</a>]</c:if>
						<c:if test="${status==101}">[<a
								href="hotThemePn_changeStatus.action?id=${id}&status=100"
								title="确认【上线】" target="ajaxTodo" style="color: blue">恢复冻结</a>]</c:if>
						[<a href="hotThemePn_seeHotThemePnDetials.action?id=${id}" title="查看"
						target="dialog" width="550" height="300" rel="input"
						style="color: blue">查看</a>]&nbsp;&nbsp;
						[<a href="hotThemePn_readyUpdate.action?id=${id}" title="更新"
						target="dialog" width="550" height="300" rel="input"
						style="color: blue">更新</a>]&nbsp;&nbsp;
						[<a href="hotThemePn_deleteHotThemePn.action?id=${id}"
