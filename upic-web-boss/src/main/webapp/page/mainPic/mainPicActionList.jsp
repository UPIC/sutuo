...
<div class="pageHeader">
	<form id="pagerForm" onsubmit="return navTabSearch(this);"
		action="banner_listMainPic.action" method="post">
		<!-- 分页表单参数 -->
		<%@include file="/page/inc/pageForm.jsp"%>
		<div class="searchBar">
			<table class="searchContent">
				<tr>
				<li><a href="banner_readyInsert.action" title="查看"
									target="dialog" width="550" height="300" rel="input"
									style="color: blue">banner添加</a></li>
				</tr>
			</table>
		</div>
	</form>
			...
			<s:iterator value="recordList" status="st">
				<tr target="sid_user" rel="${id}">
					...
					<td><c:if test="${status==100}">[<a
								href="banner_changeStatus.action?id=${id}&status=101"
								title="确认【下架】" target="ajaxTodo" style="color: blue">下架</a>]</c:if>
						<c:if test="${status==101}">[<a
								href="banner_changeStatus.action?id=${id}&status=100"
								title="确认【上价】" target="ajaxTodo" style="color: blue">上架</a>]</c:if>
						[<a href="banner_readyUpdate.action?id=${id}" title="查看"
						target="dialog" width="550" height="300" rel="input"
						style="color: blue">查看</a>]&nbsp;&nbsp;
						[<a href="banner_deleteByMainPicId.action?id=${id}"
