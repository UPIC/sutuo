...
public class BannerAction extends BossBaseAction implements ModelDriven<MainPic> {
	private static final long serialVersionUID = 1L;
	private static final Log log = LogFactory.getLog(BannerAction.class);
	...
	public String listMainPic() {
		try {
			...
			super.pageBean = mainPicService.listPage(getPageParam(), paramMap);
			super.putData("mainPic", mainPic);
	...
	public String readyUpdate() {
		try {
			MainPic byId = mainPicService.getById(mainPic.getId());
			if (byId == null) {
				return operateError("获取数据失败");
			}
	...
	public String updateMainImg() {
		try {
			...
			long result = mainPicService.update(mainPic);
			if (result == 0) {
	...
	public String changeStatus() {
		try {
			MainPic y = mainPicService.getById(mainPic.getId());
			y.setStatus(mainPic.getStatus());
			long update = mainPicService.update(y);
			if (update == 0) {
	...
	public String insert() {
		try {
			...
			mainPic.setUrl(
					"http://" + PublicConfig.BUCKETNAME_IMG + "." + PublicConfig.ENDPOINT_SHANGHAI + "/" + imgId);
			long result = mainPicService.insert(mainPic);
			if (result == 0) {
				OOSUtils.deleteFile(PublicConfig.ENDPOINT_SHANGHAI, PublicConfig.ACCESSKEYID_IMG,
						PublicConfig.ACCESSKEYSECRET_IMG, PublicConfig.BUCKETNAME_IMG, imgId);
				return operateError("添加数据失败");
	...
	public String deleteByMainPicId() {
		try {
			...
			Map<String, Object> paramMap = new HashMap<String, Object>(); // 业务条件查询参数
			paramMap.put("id", mainPic.getId()); //
			mainPicService.getBy(paramMap, "deleteById");
		} catch (Exception e) {
