...
import com.upic.weixin.api.OAuthApi;
import com.upic.weixin.po.SNSUserInfo;
import com.upic.weixin.po.SexEntity;
import com.upic.weixin.po.WeixinOauth2Token;
import com.upic.weixin.util.ValidateUtil;

@Controller
public class OAuthController extends SpringMvcControllerSupport {
			...
			try {
				...
				if (ValidateUtil.isValid(openId)) {
					...
					if (user == null) {// 如果是新关注的用户 则将用户信息保存到数据库中
						...
						LOG.debug("数据库用户为空 插入用户数据 user=" + user);
					}
