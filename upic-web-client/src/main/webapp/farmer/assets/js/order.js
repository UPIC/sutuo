$(function() {
	...
	$(".am-tab-panel").click(function() {
		if ($(this).hasClass('am-active')) {
			return;
		}
		page.pageNumStr=1;
		page.numPerPageStr=10;
		page.pageCount=0;
		page.isFlag=true;
		page.pageType=$(".am-tab-panel.am-active").attr("id");
		getByAjax(page.pageType, page,page.pageType);
	})
})
var page = {
	pageNumStr : 1,
...
function getByAjax(typeId, pagePo,divId) {
	...
	function jugeAndDeal(divId,data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			htmls += "<div class='txt'>[";
			htmls += data[i].orderClass == 'TOGETHER' ? "组局" : "商品";
			htmls += "]" + data[i].shopName + "<p><span>份数："
					+ data[i].shopCount + "份</span><br /> <span>总价："
					+ data[i].price + "元</span></p></div><div class='box5'>";
			htmls += "<span class='span1'>"
					+ obj[data[i].status]
					+ "</span><button type='button' class='am-btn am-radius'>查看详情</button></div></div></div>";
		}
	...
	$(window)
			.scroll(
					function() {
						...
						{
							var divIds=$(".am-tab-panel.am-active").attr("id") ;
								// 如果已经加载完毕
								if (!page.isFlag) {
									$("#notAnyMore"+divIds).show();
									return;
								}
								getByAjax(1, page,divIds);
						}
					});
}